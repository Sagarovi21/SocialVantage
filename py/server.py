...
session = udaExec.connect(method="odbc", system="tdap617t1.labs.teradata.com",username="alice", password="alice")
urls=[]
data = []
class Comments(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        session.execute("""UPDATE alice.TaskStatus SET pages_found = ?
        ...
        for url in self.urls:
            ...
            try:
                self.data = ()
                session.execute("""UPDATE alice.TaskStatus SET pages_completed = ? , task_status = ?  WHERE task = ? and task_id = ? """,
        ...
        session.execute("""UPDATE alice.TaskStatus SET comments_found = ?, task_status = ?
                                WHERE task = ? and task_id = ? """,
    ...
    async def runQuery(self,data):
        try:
            session.execute("""INSERT INTO alice.reviews (task_id, ObjectName, Review, Rating, TotalRating, product_details, technial_details,feature_names, feature_values )
                        VALUES (?, ?, ?, ?, ?, ? , ? , ? , ?)""",
                        data)
            session.commit()
            print(data)
    ...
    def taskQuery(self):
        session.execute("""INSERT INTO alice.taskstatus (task_id, task, ObjectName, pages_found, pages_completed, comments_found, task_status)
                        VALUES (?, ?, ?, ?, ?,?,?)""",
